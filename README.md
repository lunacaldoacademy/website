...
- ~~**地図**~~: **対応済み（2026-07-30）。** 2校とも Googleビジネスプロフィール（GBP）の
  公式埋め込みコードを使っています。地図内の「マップで開く」を押すと各校のGBPが開きます。
  差し替えるときは、Googleマップで店舗を検索 →「共有」→「地図を埋め込む」でコードを取得し、
  `src` の値だけを入れ替えてください（`title` `loading` `referrerpolicy` は既存のまま残す）
  - 住所検索（`?q=住所&output=embed`）に戻すと、**ピンが立たない／GBPに繋がらない**ので注意。
    住所検索方式では番地まで正確でないとGoogleが場所を特定できません
